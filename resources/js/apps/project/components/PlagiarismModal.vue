    ...
    <div v-if="open" class="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <div class="relative z-10 flex max-h-[85vh] w-full max-w-md flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white p-6 shadow-2xl dark:border-neutral-800 dark:bg-neutral-950">
            <div class="flex shrink-0 items-start justify-between">
                <div>
                    <h2 class="text-lg font-semibold">Screening Plagiarism</h2>
                    <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Target kemiripan Turnitin di bawah 20%.</p>
                </div>
                <button
                    type="button"
            ...
            <div v-if="loading" class="mt-5 flex flex-col items-center gap-3 py-8 text-center text-sm text-neutral-500 dark:text-neutral-400">
            ...
            <div v-else-if="result" class="mt-5 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
                <div class="rounded-lg border border-neutral-200 p-4 text-center dark:border-neutral-800">
                    ...
                    <p class="text-xs text-neutral-500 dark:text-neutral-400">Tingkat kemiripan (target &lt; 20%)</p>
                </div>

                <div v-if="result.matches.length" class="space-y-3">
                    <p class="text-xs font-medium text-neutral-500 dark:text-neutral-400">Bagian terdeteksi &amp; saran perbaikan</p>
                ...
                <p v-else class="text-xs text-neutral-400 dark:text-neutral-500">Tidak ada teks yang terdeteksi.</p>
            </div>
        </div>
    </div>
</template>
